      ...
      [`(let ([,x (env-ref ,y ,n)]) ,e0)
       ...
       (apply string-append
              `(,(comment-line
                ...
                ,(e->llvm e0)))]
      [`(let ([,x (prim ,op ,ys ...)]) ,e0)
       (define xptr (string-append (s-> x) "ptr"))
       (string-append
        (comment-line
         "  %" xptr " = alloca i64, align 8"
         (string-append "allocate result pointer for " (prim-name op) " call"))
        (comment-line
        ...
        (comment-line
         "  store volatile i64 %" (s-> x) ", i64* %" xptr ", align 8"
         (string-append "store result in result pointer"))
        (e->llvm e0))]
      [`(let ([,x (apply-prim ,op ,y)]) ,e0)
       (define xptr (string-append (s-> x) "ptr"))
       (string-append
